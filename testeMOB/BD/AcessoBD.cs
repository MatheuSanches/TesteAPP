...
namespace testeMOB
{
    public class AcessoDB : IDisposable
    {
        ...
        public void AddUser(Usuario usuario)// Inclui um Cliente no banco de dados
        {
        ...
        //    var d1 = data.Where(x => x.Nome == user.Nome && x.Usu == user.Usu).FirstOrDefault();
        //    if (d1 == null)
        //    {
        ...
        {
            return (from u in conexaoSQLite.Table<Usuario>()
                    select u).ToList();
        }
        ...
        {
            var data = conexaoSQLite.Table<Usuario>();
            var d1 = data.Where(x => x.Usu == usuario && x.Senha == senha).FirstOrDefault();
            if (d1 != null)
            {
                return true;
            }
